...
**A failed read leaves nothing behind but the cursor.** Besides the cursor, a reader holds the nesting count that enforces the depth limit and the stack of members an internally tagged object put off reading until after its payload. `rewind` takes only an offset and can restore neither, so every reader that enters a level leaves it on every exit, errors included, and an internally tagged read that fails puts back the depth and the stack it found. `tests/speculate.rs` retries failing reads through each reader that enters a level, in both formats, twice the limit's worth of times with a `rewind` between, and requires every attempt to fail with the same code at the same offset: a count that kept even one level per failure reaches the limit within the run, and one refused a level early still says `ExceededMaxDepth`, just further up. A speculating reader then reads a late-tagged object that fails beside one that does not, which is the case where a held run left on the stack would be read into the wrong object.
